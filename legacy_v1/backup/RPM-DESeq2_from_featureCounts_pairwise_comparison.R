...
args <- c("/groups/berger/user/pierre.bourguet/genomics/RNAseq/ddm1_1st_7th_ribozero_STAR/", "Col0", "ddm1_G1", "/groups/berger/user/pierre.bourguet/genomics/scripts/bam_to_kallisto/info_ddm1_1st_7th_ribozero.tab", "single-end")
# args <- c("/groups/berger/user/pierre.bourguet/genomics/RNAseq/h2aw-2_ribovanish/STAR_PE/", "WT", "w2", "/groups/berger/user/pierre.bourguet/genomics/scripts/bam_to_kallisto/info_h2aw2_ribovanish.tab", "paired-end")
...
DEG_heatmap <- function(x, y, z) {
  ...
  pheatmap(sampleDistMatrix, col=colors, filename=paste0("DEGs_batch/", "heatmap_", y, ".pdf"), main=paste0(y, "\nn=", length(x)))
}
mapply(FUN = DEG_heatmap, x=DEGs, y=paste0(names(DEGs), "_mean"), MoreArgs = list(z=cts_summary_TPM_mean[,9:(9+length(conditions))]) )
...
# export DEG tables
dirname <- paste("DEGs_AS_", args[3], "_vs_", args[2], "/", sep="")
ifelse(!dir.exists(dirname), dir.create(dirname), FALSE)
